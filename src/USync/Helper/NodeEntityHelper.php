...
class NodeEntityHelper extends AbstractEntityHelper
{
    ...
    protected function doSync($name, array $object)
    {
        ...
        if (empty($info['name'])) {
            $this->context->logWarning(sprintf('%s has no name', $name));
            $info['name'] = $name;
        }
    ...
    public function deleteExistingObject($path)
    {
        ...
        if ($exists) {
            $this->context->logDataloss(sprintf("%s node type has nodes"));
        }
    ...
    {
        if (!$this->exists($path)) {
            $this->context->logCritical(sprintf("%s node types does not exist"));
        }
